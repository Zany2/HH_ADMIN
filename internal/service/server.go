...
// MenuList 服务监控查询
func (s *sServer) ServerList(ctx context.Context, req *api.ServerListReq) (res *rr.CommonRes, err error) {
	...
	gtime.Date()
	goStartTime := gtime.Now() //启动时间

	goRunTime := gtime.Now().Timestamp() - consts.StartTime.Timestamp() //运行时长（秒）
